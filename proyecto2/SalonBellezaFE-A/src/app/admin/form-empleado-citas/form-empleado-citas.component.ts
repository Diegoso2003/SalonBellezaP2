...
@Component({
  selector: 'app-form-empleado-citas',
  standalone: true,
  imports: [NgClass, ReactiveFormsModule],
  templateUrl: './form-empleado-citas.component.html',
  styleUrl: './form-empleado-citas.component.scss'
})
export class FormEmpleadoCitasComponent {
  empleadoCitasForm: FormGroup;
  ...
  constructor(private formBuilder: FormBuilder){
    this.empleadoCitasForm = this.formBuilder.group(
      {
    ...
    this.validador = new Validador(this.empleadoCitasForm);
  }

  enviar(){
    this.informeCambio.vaciarFechas();
    if(this.empleadoCitasForm.valid){
      this.mandarInfo();
    } else {
  ...
  private mandarInfo(): void {
    ...
    this.adminService.obtenerCitasPorEmpleado(consulta).subscribe(
      {
        ...
        , error: (error) => {
          this.informacion.informarError(error.error.mensaje || 'Error al obtener el reporte de citas por empleado');
        }
      }
    );
  }
  ...
  private armarConsulta(): Consulta {
    const fechaInicio = this.empleadoCitasForm.get('fechaInicio')?.value;
    const fechaFin = this.empleadoCitasForm.get('fechaFin')?.value;
    const dpi = this.empleadoCitasForm.get('dpi')?.value;
